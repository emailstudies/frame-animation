/* this is for all folders - SELECTED */

function getFoldersAndMaxFrames(doc) {
  var folders = [];
  var maxFrames = 0;
...
function exportGifFromSelected() {
  ...
  const script = `
    (function () {
      ...
      // ✅ Step 1: Gather selected folders (LayerSets) – any name except anim_preview
      var selected = [];
      ...
      var data = (${getFoldersAndMaxFrames.toString()})(doc);
      ...
      // 🧹 Step 5: Remove all folders except anim_preview
      for (var i = doc.layers.length - 1; i >= 0; i--) {
        var layer = doc.layers[i];
        if (
          layer.typename === "LayerSet" &&
...
/* this is for selected anim_ folders
function exportGifFromSelected() {
  const fps = getSelectedFPS();
  const manual = document.getElementById("manualDelay").value;
  const delay = manual ? Math.round(parseFloat(manual) * 1000) : fpsToDelay(fps);
